"""
赶集直招爬虫。
搜索页：https://cd.ganji.com/zhaopin/keyword/
"""
...
class GanjiScraper(BaseScraper):
    ...
    search_url = "https://cd.ganji.com/zhaopin/{keyword}/o{page}/"
    ...
    def _parse_html(self, html: str, round_label: str) -> list[Job]:
        ...
        items = tree.css("div.job-item") or tree.css("li.list-item")

        for item in items:
            try:
                title_el = item.css_first("a.title, a.job-title, dt a")
                if not title_el:
                    continue
