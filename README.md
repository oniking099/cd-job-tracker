...
13 个招聘平台，全部通过 LLM Agent 模拟人类操作采集（observe→think→act），优先用 URL 城市参数直达成都，数据层 `_filter_city` 二次兜底：
...
> **登录墙说明**：BOSS/拉勾/58/鱼泡 有平台级登录/验证码风控，Agent 无法绕过（不提供账号凭据），适配器会在识别到登录墙时快速跳过，不浪费 API 预算。其余 9 平台实测均能提取成都岗位。
